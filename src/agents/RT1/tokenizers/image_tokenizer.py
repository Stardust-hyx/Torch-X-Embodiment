...
from ..film_efficientnet.pretrained_efficientnet_encoder import (
    EfficientNetEncoder,
)
...
class RT1ImageTokenizer(nn.Module):
    ...
    @torch.no_grad()
    def tokenize_single_img(
        self, image: torch.Tensor, context: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
        """Gets tokens of an image.

        Args:
        image: Image of shape (1, 3, h, w) to tokenize.
        context: An optional context vector (e.g., a natural language embedding).
            Expected to have shape (1, embedding_dim).

        Returns:
        tokens: has shape (1, num_tokens_per_timestep, embedding_dim)
        """

        tokens = self._tokenizer(image, context=context)  # [1, 512 , 10, 10]

        if self._use_token_learner:
            tokens = self._token_learner(tokens)  # [1, num_token, 512]
            return tokens
        else:
            tokens = tokens.view(1, 512, -1)  # [1, 512 , 10 * 10]
            # If you don't use token learner, the number of token is 100.
            tokens = tokens.transpose(1, 2)  # [1, 10 * 10, 512]
            return tokens