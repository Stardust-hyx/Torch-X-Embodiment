...
class RT1Agent(nn.Module):
    ...
    def __init__(
        ...
        if self.augment and self.augment_kwargs:
            ...
            self.augment_kwargs['random_resized_crop']['scale']  = (0.9, 1.0) #TODO
    ...
    def forward(self, prompts, obs_imgs, goal_imgs, feature_imgs):
        ...
        dist = MultivariateNormal(action_logits.float(), scale_tril=self.fixed_std.float())
    ...
    def sample_actions(self, prompt, obs_img, pre_context_image_tokens=None, argmax=True):
        ...
        obs_img = self.img_resize(obs_img)
        obs_img = self.img_normalize(obs_img)

        context = self._encoding_prompt([prompt]) # [1, embedding_dim]
        obs_img_tokens = self._image_tokenizers.tokenize_single_img(obs_img, context) # [1, num_tokens, embedding_dim]
        obs_img_tokens = obs_img_tokens.unsqueeze_(0) # [1, 1, num_tokens, embedding_dim]

        pre_length = pre_context_image_tokens.size(1) if pre_context_image_tokens is not None else 0
        # context_image_tokens: (1, t, num_tokens, embedding_dim)
        if pre_length == self._time_sequence_length:
            context_image_tokens = torch.cat((pre_context_image_tokens[:, 1:], obs_img_tokens), dim=1)
            attention_mask = self._default_attention_mask
            action_tokens_mask = self._action_tokens_mask
        elif pre_context_image_tokens is not None:
            context_image_tokens = torch.cat((pre_context_image_tokens, obs_img_tokens), dim=1)
            num_all_tokens = (pre_length+1) * self._single_time_step_num_tokens 
            attention_mask = self._default_attention_mask[:num_all_tokens, :num_all_tokens]
            action_tokens_mask = self._action_tokens_mask[:(pre_length+1) * self._tokens_per_action]
        else:
            context_image_tokens = obs_img_tokens
            attention_mask = self._default_attention_mask[:self._single_time_step_num_tokens , :self._single_time_step_num_tokens]
            action_tokens_mask = self._action_tokens_mask[:self._tokens_per_action]
        ...
        # Gather all predicted actions for the action loss. Use fancy index to extract all predicted actions.
        predicted_action_index = torch.tensor(action_tokens_mask) - 1
        action_logits = output_tokens[
            :, predicted_action_index
        ]  # (bs, t*tokens_per_action, 1)
        action_logits = action_logits.view(
        ...
        if argmax:
            ...
            actions = dist.sample()
        return actions, context_image_tokens
    ...
    # output context from observation. size: [b, emb-size]
    ...
    def renew_action_linear(self, action_dim):
        if action_dim != self._tokens_per_action:
            self._tokens_per_action = action_dim
            self.register_buffer("fixed_std", torch.eye(action_dim))
    
    def set_feature_layers_require_grad(self, flag: bool):
        self._image_tokenizers.requires_grad_(flag)
